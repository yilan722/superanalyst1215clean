    ...
    try {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          // 如果无法解析JSON，使用默认错误
          errorData = { error: response.statusText || '未知错误' }
        }
        ...
        // 提取错误详情
        const errorMsg = errorData.details || errorData.error || (currentLocale === 'zh' ? '报告生成失败' : 'Report generation failed')
      ...
      // 提供更友好的错误信息
      let errorMessage = currentLocale === 'zh' ? '报告生成失败，请稍后重试' : 'Report generation failed, please try again later'
      ...
      if (error instanceof Error) {
        ...
        if (errorMsg.includes('超时') || errorMsg.includes('timeout')) {
          errorMessage = currentLocale === 'zh' 
            ? '请求超时，请稍后重试' 
            : 'Request timeout, please try again later'
        } else if (errorMsg.includes('网络') || errorMsg.includes('network') || errorMsg.includes('fetch')) {
          errorMessage = currentLocale === 'zh' 
            ? '网络连接问题，请检查网络后重试' 
            : 'Network connection issue, please check your connection and try again'
        } else if (errorMsg.includes('API') || errorMsg.includes('403') || errorMsg.includes('429') || errorMsg.includes('500')) {
          errorMessage = currentLocale === 'zh' 
            ? '服务暂时不可用，请稍后重试' 
            : 'Service temporarily unavailable, please try again later'
        } else if (errorMsg.includes('深度分析')) {
          // 使用服务器返回的详细错误信息
