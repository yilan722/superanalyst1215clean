    ...
    try {
      ...
      try {
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
          try {
            if (attempt > 1) {
              const waitTime = Math.min(1000 * Math.pow(2, attempt - 2), 5000) // 指数退避，最多5秒
              console.log(`🔄 重试深度分析 (尝试 ${attempt}/${maxRetries})，等待 ${waitTime}ms...`)
              await new Promise(resolve => setTimeout(resolve, waitTime))
            }
            
            analysisResult = await deepAnalyst.generateValuationReport(
              companyName,
              formattedInfo,
              'comprehensive',
              locale
            ...
            if (analysisResult.status === 'success' && analysisResult.reportJson) {
              console.log(`✅ 深度分析完成 (尝试 ${attempt}/${maxRetries})`)
              break
            } else {
              const errorMsg = analysisResult.error || '未知错误'
              lastError = new Error(`深度分析失败: ${errorMsg}`)
              console.warn(`⚠️ 深度分析失败 (尝试 ${attempt}/${maxRetries}): ${errorMsg}`)
            ...
            console.warn(`⚠️ 深度分析异常 (尝试 ${attempt}/${maxRetries}): ${errorMsg}`)
            
            // 如果是最后一次尝试，抛出错误
            if (attempt === maxRetries) {
              // 提供更详细的错误信息
              const detailedError = error.message || '未知错误'
              const isTimeout = errorMsg.includes('超时') || errorMsg.includes('timeout') || errorMsg.includes('AbortError')
              const isNetworkError = errorMsg.includes('网络') || errorMsg.includes('network') || errorMsg.includes('fetch')
              const isApiError = errorMsg.includes('API') || errorMsg.includes('403') || errorMsg.includes('429')
              
              let userFriendlyError = '深度分析失败'
              if (isTimeout) {
                userFriendlyError = '深度分析超时，请稍后重试'
              } else if (isNetworkError) {
                userFriendlyError = '网络连接问题，请检查网络后重试'
              } else if (isApiError) {
                userFriendlyError = 'API服务暂时不可用，请稍后重试'
              } else {
                userFriendlyError = `深度分析失败: ${detailedError}`
              }
        ...
        // 最终验证
        if (!analysisResult || analysisResult.status !== 'success' || !analysisResult.reportJson) {
          const errorMsg = analysisResult?.error || lastError?.message || '未知错误'
          console.error(`❌ 深度分析最终失败: ${errorMsg}`)
