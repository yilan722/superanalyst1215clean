...
Start directly with the opening brace. DO NOT forget table format!`
    ...
    try {
      ...
      // 提供更友好的错误消息
      if (errorMsg.includes('超时') || errorMsg.includes('timeout') || errorMsg.includes('AbortError')) {
        errorMsg = '深度分析请求超时，请稍后重试'
      } else if (errorMsg.includes('网络') || errorMsg.includes('network') || errorMsg.includes('fetch')) {
        errorMsg = '网络连接问题，请检查网络后重试'
      } else if (errorMsg.includes('API') || errorMsg.includes('403') || errorMsg.includes('429')) {
        errorMsg = `API服务错误: ${errorMsg}`
      }
