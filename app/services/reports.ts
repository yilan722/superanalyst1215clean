...
export async function getAllReports(): Promise<Report[]> {
  try {
    ...
    if (fs.existsSync(todaysReportPath)) {
      ...
      if (todaysReport && todaysReport.id) {
    ...
    if (fs.existsSync(historicalReportsPath)) {
      ...
      const historicalReportsList = historicalReports.reports || historicalReports || []
      
      // 合并历史报告，避免重复
      for (const report of historicalReportsList) {
        if (report && report.id && !reports.find(r => r.id === report.id)) {
          reports.push(report)
        }
      }
    }
