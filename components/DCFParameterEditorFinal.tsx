...
interface DCFParameterEditorFinalProps {
  initialParameters: DCFParametersFinal
  onParametersChange: (parameters: DCFParametersFinal) => void
  onRecalculate: (parameters: DCFParametersFinal) => void
...
export default function DCFParameterEditorFinal({
  initialParameters,
  onParametersChange,
  onRecalculate,
  isRecalculating,
  ...
  ticker = 'BAC'
}: DCFParameterEditorFinalProps) {
  const [parameters, setParameters] = useState<DCFParametersFinal>(initialParameters)
  ...
  // 更新参数
  const updateParameter = (key: keyof DCFParametersFinal, value: any) => {
      ...
      <div className="px-6 py-4 border-b border-gray-200 bg-gray-50">
        ...
        <div className="flex space-x-1 border-b border-gray-200">
          <button className="px-4 py-2 text-sm font-medium text-gray-600 hover:text-gray-800">
            {isChinese ? '估值仪表板' : 'Valuation Dashboard'}
          </button>
          <button className="px-4 py-2 text-sm font-medium text-blue-600 border-b-2 border-blue-600">
            {isChinese ? 'DCF & 反向DCF' : 'DCF & Reverse DCF'}
          </button>
          <button className="px-4 py-2 text-sm font-medium text-gray-600 hover:text-gray-800">
            {isChinese ? '格雷厄姆公式' : "Graham's Formula"}
          </button>
          <button className="px-4 py-2 text-sm font-medium text-gray-600 hover:text-gray-800">
            {isChinese ? 'EBIT倍数' : 'EBIT Multiples'}
          </button>
          <button className="px-4 py-2 text-sm font-medium text-gray-600 hover:text-gray-800">
            {isChinese ? '更多' : 'More'}
          </button>
      ...
      <div className="p-6">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-lg font-semibold text-gray-800">
            {isChinese ? '更新DCF & 反向DCF输入' : 'UPDATE DCF & REVERSE DCF INPUTS'}
          </h3>
          <div className="flex items-center space-x-4">
            <button
              onClick={handleReset}
